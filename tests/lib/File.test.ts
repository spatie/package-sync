...
it('reads a file', () => {
    ...
    expect(f.contents.trim()).toStrictEqual('test one');
});
...
it('can set its contents without saving', () => {
    ...
    expect(f.contents).toStrictEqual('two');
    expect(readFileSync(filename, { encoding: 'utf-8' }).trim()).toStrictEqual('test one');
});
...
it('can save its contents to another file', () => {
    ...
    expect(existsSync(destFilename)).toBeFalsy();

    File.read(srcFilename).saveAs(destFilename);

    expect(existsSync(destFilename)).toBeTruthy();
    expect(readFileSync(destFilename, { encoding: 'utf-8' }).trim()).toStrictEqual('test one');
...
it('can create a new file', () => {
    ...
    const f = File.create(fn).setContents('new file')
        .save();

    expect(readFileSync(fn, { encoding: 'utf-8' }).trim()).toStrictEqual(f.contents);
...
it('can delete a file', () => {
    ...
    expect(existsSync(fn)).toBeFalsy();
});
...
it('can check if a file exists', () => {
    ...
    expect(File.create(fn1).exists()).toBeTruthy();
    expect(File.create(fn2).exists()).toBeFalsy();
});
...
it('can get the size of a file on disk', () => {
    ...
    expect(File.create(fn1).sizeOnDisk).toBe(9);
    expect(File.create(fn2).sizeOnDisk).toBe(0);
});

it('processes files with template variables', () => {
    const fn = dataFilename('template.txt');

    expect(File.create(fn).processTemplate('my-package')).toMatchSnapshot();
});
