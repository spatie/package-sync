...
import { fileSize, isDirectory, last, matches, uniqueArray, uniqueStrings } from '../../src/lib/helpers';
...
it('matches a string against patterns', () => {
    expect(matches('test', ['t*', '*t'])).toBeTruthy();
    expect(matches('test', 'a*')).toBeFalsy();
    expect(matches('test', '*')).toBeTruthy();
    expect(matches('', '*')).toBeFalsy();
});

it('matches an array of strings against patterns', () => {
    expect(matches(['foo', 'bar'], ['f*', 'b*'])).toBeTruthy();
    expect(matches(['far', 'bar'], 'b*')).toBeTruthy();
    expect(matches(['test', 'abc'], 'te*')).toBeTruthy();
    expect(matches(['foo', 'bar'], 'x*')).toBeFalsy();
    expect(matches(['far', 'bar'], '!*ar')).toBeFalsy();
    expect(matches(['foo', 'bar'], '*')).toBeTruthy();
});

it('gets the size of a file', () => {
    expect(fileSize(`${__dirname}/../data/testfile1.txt`)).toBe(9);
    expect(fileSize(`${__dirname}/../data/missing.txt`)).toBe(0);
    expect(fileSize(__dirname)).toBe(0);
});
